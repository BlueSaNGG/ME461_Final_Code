...
uint16_t read_lidar_state = 0; //states for lidar reading, initialized as wait for S command
char G_command[] = "G04472503\n"; //command for getting distance -120 to 120 degree
uint16_t G_len = 11; //length of command
datapts lidar_data[228]; //distance data from lidar
xy lidar_pts[228]; //xy data
float LIDARrightfront = 0;
float LIDARfront = 0;
uint16_t LIDARpingpong = 0;
float LIDARtemp_x = 0;
float LIDARtemp_y = 0;
...
__interrupt void RXCINT_recv_ready(void)
{
    ...
    if (RXCdata & 0xC000) {
        ...
        if (read_lidar_state == 0) {//wait for the starting char 'G'
            buf_index = 0;
            if (RXCdata =='G') {
                read_lidar_state = 1;
                G_count = 1;
            }
        } else if (read_lidar_state == 1) {//get the headers
        ...
        } else if (read_lidar_state == 2) {//get actual data
            if (RXCdata == 0x0a) {//check for line feed
                ...
                if (get_dis_first == 1) {
                    ...
                    if (LIDARpingpong == 0) {
                        //lidar_data[dis_index%228].distance_ping = distance*0.00328084; // fill in the distance_ping array
                        checkdist = distance*0.00328084;
                        if ((checkdist > 0.0) && (checkdist < 20.0)) {
                            lidar_data[dis_index].distance_ping = checkdist; // fill in the distance_ping array
                        } else {
                            numRCXerrors++;
                            lidar_data[dis_index].distance_ping = lidar_data[dis_index].distance_pong;
                        }
                    } else if (LIDARpingpong == 1) {
                        //lidar_data[dis_index%228].distance_pong = distance*0.00328084; // fill in the distance_pong array
                        checkdist = distance*0.00328084;
                        if ((checkdist >= 0.0) && (checkdist < 20.0)) {
                            lidar_data[dis_index].distance_pong = checkdist; // fill in the distance_ping array
                        } else {
                            numRCXerrors++;
                            lidar_data[dis_index].distance_pong = lidar_data[dis_index].distance_ping;
                        }
                    }
                    dis_index += 1;
                    get_dis_first = 1;
